...
class main
{
	...
	{
		$this->language->add_lang('andreask/ium', 'user_self_delete_page');
		...
		{
			return $this->helper->error($this->language->lang('PAGE_NOT_EXIST'), 404);
		}

		// Check to see if user is logged in.
		if ($this->user->data['user_id'] == ANONYMOUS)
		{
			return $this->helper->error(login_box('', $this->language->lang('HAVE_TO_LOGIN')), 403);
		}

		if (! $this->user_check( $this->user->data['user_id'], $random ) )
		{
			return $this->helper->error($this->language->lang('INVALID_LINK_OR_USER'), 403);
		}
		...
		if ($this->request->is_set_post('submit') )
		{
			...
			{
				$this->helper->error($this->language->lang('FORM_INVALID') . $this->usr_back_link( $this->u_action ), 403);
			}
			// Make sure confirm is selected
			if ( $this->request->variable('self_delete_verify', '') != 1)
			{
				return $this->helper->message($this->language->lang('HAVE_TO_VERIFY') . $this->usr_back_link( $this->u_action ));
			}
			...
			$message = ($this->config['andreask_ium_approve_del']) ? $this->language->lang('NEEDS_APPROVAL', htmlspecialchars_decode($this->config['sitename'])) : $this->language->lang('USER_SELF_DELETE_SUCCESS', htmlspecialchars_decode($this->config['sitename']));
		...
		return $this->helper->render('ium_user_remove.html', $this->language->lang('USER_SELF_DELETE_TITLE'));
	}
